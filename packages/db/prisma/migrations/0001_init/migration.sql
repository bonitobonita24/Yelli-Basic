...
-- CreateEnum
CREATE TYPE "ExportJobStatus" AS ENUM ('queued', 'processing', 'ready', 'expired');
...
-- Super-Admin uses a dedicated Prisma client without the L6 extension AND must bypass
-- RLS — either by BYPASSRLS role attribute or by setting app.current_tenant_id
-- to a sentinel value matched by an additional policy (deferred to Phase 5/deploy role config).
