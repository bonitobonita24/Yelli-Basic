...
function makeCallerThatSucceeds(ctx: TestCtx = {}) {
  ...
  const proc = t.procedure.use(errorMiddleware).query(() => ({ ok: true }));
  const r = t.router({ ping: proc });
  return r.createCaller(ctx);
}

function makeCallerThatFails(ctx: TestCtx = {}, code: TRPCError['code'] = 'INTERNAL_SERVER_ERROR', cause?: Error) {
  const t = initTRPC.context<TestCtx>().create({ transformer: superjson });
  const proc = t.procedure.use(errorMiddleware).query(() => {
...
describe('errorMiddleware — structured logging on error', () => {
  ...
  it('re-throws the error unchanged (does not swallow)', async () => {
    ...
    await expect(caller.ping()).rejects.toMatchObject({ code: 'FORBIDDEN' } satisfies Partial<TRPCError>);
  });
