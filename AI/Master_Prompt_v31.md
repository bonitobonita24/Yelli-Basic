...
>                        Opus dispatches Sonnet via Agent(model: "sonnet") for scoped tasks.
>
...
Claude.ai in the Planning Assistant — Phase 3.3 runs in Claude Code.)
...
+ schema from Phase 3), then dispatches Sonnet to execute per the R6 size rule (read DESIGN.md ≤200
lines direct, else Governance-Extraction Scout per R6/V32.3). Sonnet: `/design-tokens` EXPAND →
...
**Full specification:** See `.claude/rules/phases.md` — Phase 3.3 section (steps + output contract +
...
**Full specification:** See `.claude/rules/phases.md` — Phase 4 anti-thrashing section.
...
**Full specification:** See `.claude/rules/phases.md` — Phase 8 anti-thrashing section.
