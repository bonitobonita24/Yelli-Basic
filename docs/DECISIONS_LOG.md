...
- [swarm W8 · 2026-06-13 08:48:52] 2026-06-13 W8 deferred by Brain (q-run31-W8-01, bucket A): prerequisites W1b/W2b-2/accounts-auth authorize()+invitation.accept/W5-runtime/W5b-e/W6b open per STATE.md NEXT 4/8/9; W8 to be re-dispatched as final validation once wire sessions land. Governing: Master Prompt Phase 5/6 gates + anti-thrashing (phases.md, memory-governance.md §1).
- [swarm W8 · 2026-06-13 08:49:08] Phase 5 audit gate: esbuild HIGH (Deno-only, GHSA binary-integrity) accepted-with-mitigation; .npmrc audit-level=critical; Brain bucket A, Master Prompt Phase 5 CVE decision-tree §accept-with-mitigation; Yelli-Basic W8 q-run31-W8-02.
- [swarm W8 · 2026-06-13 08:53:00] 2026-06-13 W8 run-32 DEFERRED again (q-run32-W8-01, bucket A, reaffirms q-run31-W8-01 [A]). HEAD still at W7 (1fd6dbc); STATE.md NEXT items 4/8/9 OPEN. Dispatch order: W1b → W2b-2 → accounts-auth (authorize()+invitation.accept) → W5-runtime/W5b-e → W6b, then re-dispatch W8 as true final validation. Build-only override rejected (violates Phase 5/6 gate + Rule 1). esbuild accept-with-mitigation (q-run31-W8-02) applies during true W8.
- [swarm S0 · 2026-06-13 15:40:12] S0/q-S0-01: Resolved A (high) — User.passwordHash uses bcryptjs@12 per LOCKED DECISIONS_LOG L125-129; authorize() + invitation.accept both use bcrypt. PRODUCT.md L198 Argon2id parenthetical flagged as stale doc bug for human-authored Rule-1 correction. Argon2id remains reserved for Tenant.adminPassphraseHash only. No re-seed; existing CREDENTIALS.md webmaster row stands.
- [swarm S0 · 2026-06-13 15:40:27] S0/q-S0-02 [B]: authorize() tenant resolution uses form-supplied credentials.tenantSlug (lookup by slug); subdomain/path resolver deferred. LAN mode submits slug='default'. Source: PRODUCT.md LAN deployment + existing scaffold config.ts L20-22.
- [swarm S0 · 2026-06-13 15:40:47] S0/q-S0-03 [A]: LAN admin sign-in implemented as dedicated POST /admin/login Route Handler + signInLanAdmin() in lan-admin.ts; NOT a second Auth.js Credentials provider. Rationale: PRODUCT.md L73+L386 lock the route as distinct from /api/auth/*; LAN_ADMIN_COOKIE (yelli_admin_session) is owned by lan-admin.ts; rate-limit 5/min/IP enforced at Route Handler boundary. Auth.js config.ts authorize() scoped to Cloud/LAN-account user auth only.
- [swarm S2 · 2026-06-13 16:03:34] q-80-S2-01 [A/high] S2 re-scoped to W5-runtime only (Worker host + device-archive cron + compose service + Dockerfile.workers); W5b-W5e dispatched as separate ordered sub-sessions per LOCKED q-W5-03 and V32 anti-thrashing budget.
- [swarm S2 · 2026-06-13 16:03:49] q-80-S2-02 [A,high]: soft-delete-cron deferred from S2; stub retains `throw NotImplemented`. Honors LOCKED q-W5-01 (STATE.md:191) requiring separate schema session for User.removedAt + removeMember before sweep wiring.
- [swarm S2 · 2026-06-13 16:06:46] 2026-06-13 q-81-S2-01 [A/high] S2 worker decomposition approved per W5a STATE.md lock: dispatch as W5-runtime → W5b-tenant-export → W5c-email → W5d-logo-image → W5e-backup, one session each, ≤500 LOC / ≤12 files / ≤80K tokens, single new dep max per session. Governing: Master Prompt Tiered Decomposition (Tier 3 mandatory split) + V32 R1/R5 + V32.2 R6 + §2 Smart Checkpoint. Brain resolved (bucket A).
- [swarm S2 · 2026-06-13 16:07:00] q-81-S2-02 [A/high]: soft-delete-cron removed from S2 scope; stays stubbed (NotImplemented). Deferred behind locked q-W5-01 (schema session: add User.removedAt, define removeMember w/ Invitation FK + AuditLog immutability resolution). Sweep implementation to be authored in subsequent session after schema lands.
- [swarm S3a · 2026-06-13 16:26:09] q-S3a-01 [A/high] Resolved S3a routing+wiring conflict: page at app/(public)/admin/login/page.tsx, S0's POST handler relocated path-only from app/admin/login/route.ts → app/api/admin/login/route.ts (no logic change). Page client-POSTs to /api/admin/login; LAN admin remains on LOCKED Step 6 cookie path (signInLanAdmin + yelli_admin_session). Session-brief phrase 'wired to next-auth signIn()' overridden by LOCKED Step 6 per H1 (DECISIONS_LOG > brief). Authority: Next.js 16 routing (route.js/page.js path collision) + Master Prompt H1 priority.
- [swarm S3d · 2026-06-13 17:14:55] q-87-S3d-01 [A/high] S3d split: ship ScreenAuditView (Flow H → trpc.audit.list, Wave-11 §11 prefix chips); defer ScreenTenantSettings pending Phase 3.3 prototype + client sign-off for branding/org-settings surface. Brief defect noted: trpc.tenants.update does not exist; only trpc.brand.update (displayName+logo). Governed by Rule 29 INHERIT-not-REPLACE + Phase 3.3 gate.
- [swarm S3d · 2026-06-13 17:15:12] q-87-S3d-02 [A/high] ScreenTenantSettings scoped to /admin/branding only (page #17) against existing trpc.brand.update; /admin/settings (page #18) deferred pending a new backend session to author tenants.update (displayName-only, slug immutable per Step 7 lock). Rationale: Rule 1 + Architect-Execute Model + Tier-3 split.
- [swarm S6 · 2026-06-13 18:10:54] 2026-06-13 q-91-S6-01 [A/high] S6 premature — STATE.md NEXT not closed (call-engine unbuilt; 5/6 BullMQ workers stubs; ScreenTenantSettings deferred). Decision: re-sequence — dispatch device-home call-engine + W5b-e queue bodies + ScreenTenantSettings, THEN re-dispatch S6. Tagging phase-4-complete against partial build refused (Rule 1 + S6 scope gate).
- [swarm S6 · 2026-06-13 18:13:49] 2026-06-13 q-92-S6-01 [A framework]: S6 validation stood down; per q-91-S6-01 [A], dispatch 6 ordered build sessions (device-home call-engine → W5b/W5c/W5d/W5e worker bodies → ScreenTenantSettings prototype+tenants.update) before re-dispatching S6. Tagging phase-4-complete against partial build prohibited (Master Prompt Phase 8 completeness check + Rule 1).
- [swarm S6 · 2026-06-13 18:16:20] q-93-S6-01 [A]: S6 stands down. Orchestrator to dispatch in order: (1) device-home call-engine, (2) W5b tenant-export body, (3) W5c email body, (4) W5d logo-image body, (5) W5e backup body, then re-dispatch S6 as real W8 validation. ScreenTenantSettings deferred to a separate session pending Phase 3.3 prototype + new tenants.update mutation (slug immutable). Governing: Master Prompt Phase 8 completeness + anti-thrashing; V32.6 Phase 3.3 hard gate; Rule 29 INHERIT-not-REPLACE.
- [swarm S6 · 2026-06-13 18:18:51] 2026-06-13 q-94-S6-01 [A/high] S6 stood down; dispatch 6 prerequisite sessions in locked order (device-home call-engine → W5b tenant-export → W5c email → W5d logo-image → W5e backup → ScreenTenantSettings w/ Phase-3.3 prototype + new trpc.tenants.update); re-dispatch S6 as real W8 end-to-end validation only after all six land. Tagging phase-4-complete against partial build prohibited per Master Prompt Phase 8 Completeness Check + anti-thrashing.
- [swarm setup · 2026-06-14 16:03:00] setup/q-setup-01 [A/high] Built the two LAN-admin mechanisms `lan-admin.ts` referenced but never shipped: (1) `/setup` first-run flow, (2) `scripts/reset-admin-passphrase.sh`. `/setup` = Route Handler `app/api/admin/setup/route.ts` mirroring `/api/admin/login` (Zod `{passphrase,confirm}`, 8-char floor, 5/min/IP rate limit, generic `{ok:false}` on every failure — no setup-state leak per security.md AUTH rule) + server-gated page `app/(public)/setup/page.tsx` (redirects to `/admin/login` when already configured) + `SetupForm.tsx` (shadcn + Clay tokens, INHERIT-not-REPLACE twin of the login page). Backend `setLanAdminPassphrase()` sets `Tenant.adminPassphraseHash` (Argon2id `hash`) ONLY first-run via atomic `updateMany(where adminPassphraseHash:null)` race-guard, audits `lan.admin.setup.success|fail` (L6-excluded base client, explicit tenantId — security.md #10), then mints `yelli_admin_session` and routes to `/admin/settings`. Authority: lan-admin.ts module contract (Step 6 LOCKED) + security.md AUTH/L6 rules + existing `/api/admin/login` precedent (q-S3a-01).
- [swarm setup · 2026-06-14 16:03:00] setup/q-setup-02 [A/high] Admin guard redirect (`app/admin/layout.tsx`) now branches on setup-state: non-admins → `/setup` when NO passphrase configured (`lanAdminConfigured()===false`, first-run), → `/admin/login` when one is. One extra read on the unauthenticated path only; Cloud is unaffected (Cloud admins are already `isAdmin`; `lanAdminConfigured()` is always false on Cloud but the setup page self-refuses with no LAN tenant). Rationale: closes the dead-end where a fresh LAN deploy bounced operators to a login they could never pass. Authority: task spec + existing layout guard pattern (resolveAppShellContext / S4 W1b).
- [swarm setup · 2026-06-14 16:03:00] setup/q-setup-03 [A/framework] Reset script handles pnpm workspace dep isolation by invoking each native dep from its OWNING package: hash via `pnpm --filter @yelli/web exec node` (owns `@node-rs/argon2`), DB update + `lan.admin.reset` audit via `pnpm --filter @yelli/db exec node` (owns `@prisma/client`) — temp helper `.cjs` files written into each package dir, trap-cleaned. Targets the earliest-created tenant (matches `resolveLanTenant` ordering, LAN single-tenant Step 1). Loads `DATABASE_URL` from `.env.dev` (or `--env-file`); arg-or-hidden-prompt passphrase (+confirm), 8-char floor, never echoes secret/hash. **Verified live against dev `_pwbt`** (reset → argon2 verify true → restored to `admin`). Rationale: a root-level or single-package `node -e` cannot resolve both deps (hoist isolation); resolved by-package is the robust idempotent path. Authority: `sync-credentials-to-env.sh` conventions + task spec pnpm-isolation note.
- [phase-8 audit · 2026-06-16] q-P8-doc-01 [A/technical]: Adversarial Phase-8 audit (router-vs-UI gap hunt + stub sweep) against `swarm/rebuild` HEAD `931d271`. Found NO autonomous feature/wire gap — all 8 tRPC routers registered in root.ts; all 6 BullMQ worker bodies real (device-archive/tenant-export/soft-delete-cron/backup/email/logo-image); 161 tests green (signaling 10 · jobs 34 · web 117), turbo cache fully bypassed. ONLY defect = two stale doc/comment claims that 5 workers "throw NotImplemented / are STUBS": (1) `packages/jobs/src/runtime/processors.ts` registry docstring, (2) `docs/IMPLEMENTATION_MAP.md` packages/jobs header. Both corrected to reflect that every worker is implemented; the two intentional fail-fast paths (`backup` w/o BACKUP_S3 = owner-deferred; `email` verify/reset kinds = Phase-7 magic-link/email-link providers per DECISIONS_LOG line 204) are documented as by-design, not unfilled stubs. Comment/doc-only change, zero source/runtime behaviour change; full typecheck+test re-run GREEN cache-forced. The `verify`/`reset` email producers + Cloud public-auth (signup/verify/reset) UI remain Phase-7 product-scoped (WHAT-to-build → owner), NOT autonomous. Authority: Phase-8 completeness check + autonomy boundary (technical-only).
- [phase-3.3 sign-off · 2026-06-17] Owner approved Phase-3.3 ScreenTenantSettings design sign-off for `/admin/settings` surface. Design gate CLOSED. Phase-4-complete tag and merge to `main` authorised. Authority: owner explicit sign-off (finishing architect session 2026-06-17).
- [v329-compliance · 2026-06-20] q-v329-01 [A/technical] V32.9 Compliance & Data Privacy Layer implemented on `feat/v329-compliance-features` (off `main`). PH Data Privacy Act (RA 10173 / NPC) feature set wired against existing multi-tenant data: new `dsr` tRPC router (`dsr.access`/`port`/`rectify`/`erase`/`myRequests`, all `protectedProcedure`, session-derived userId/tenantId, L5 AuditLog) + admin-gated `breach` router (`record`/`markNpcNotified`/`markSubjectsNotified`/`submitReport`/`list`, 72h-notify + 5-business-day-written-report due-date computed at creation per NPC Circular 16-03). Prisma models added: `ConsentLog`, `DataSubjectRequest`, `BreachNotificationRecord`, `RetentionPolicy` + enums `LawfulBasis`/`DsrType`/`DsrStatus`/`BreachStatus`/`BreachSeverity`; `AuditTargetType` extended. `dsr.erase` honors legal-retention (AuditLog 7yr / CallSession 1yr NOT purged) — reuses existing 7-day self-soft-delete semantics. 24 new vitest tests; full gate green (lint 7/7, typecheck 7/7, test 140 web/34 jobs/10 signaling, build 2/2).
- [v329-compliance · 2026-06-20] q-v329-02 [A/technical] DSR statutory response window = 15 calendar days (NPC reasonable-period default). **OWNER-RATIFIED 2026-06-21** — the "needs owner/legal confirmation" caveat is removed; 15 days is now the authoritative value (`DSR_DUE_DAYS = 15` in `dsr.ts`, already clean — no code change required). `dsr.object`/`dsr.inform` rights remain `// TODO(v329)` (require consent-withdrawal UI + stored privacy-notice versioning, future wire session — intentionally untouched).
- [v329-compliance · 2026-06-20] q-v329-03 [A/technical] Honest ComplianceFooter (`components/shell/ComplianceFooter.tsx`): truthful design-claims shown by default ("WCAG 2.2 AA-targeted", "PH Data Privacy Act aligned", /privacy + DPO mailto links); third-party cert badges (ISO 27001 / SOC 2) OFF by default behind `showCertBadges` prop (Yelli holds no such certs — flip only when real). Clay-themed, 44px targets. Rationale: V32.9 honest-badge policy (design-claims on, cert-badges off unless real).
- [v329-compliance · 2026-06-20] q-v329-04 [A/default-flagged] Rule-1 product defaults chosen (NOT written to PRODUCT.md): Controller = Powerbyte IT Solutions; DPO contact = bonitobonita24@gmail.com (hardcoded in /privacy + ComplianceFooter — update if a dedicated DPO mailbox is created). Privacy-page retention periods sourced verbatim from PRODUCT.md §10-12 (AuditLog 7yr, CallSession 1yr, account 7-day soft-delete grace). PRODUCT.md said "WCAG 2.1 AA" but V32.9 framework mandates WCAG 2.2 AA (R13 + DICT MC 004); footer/page state 2.2 AA-targeted. **OWNER-RATIFIED 2026-06-21**: PRODUCT.md reconciled 2.1→2.2 AA (NFR Accessibility line + Design Identity key-constraint) under explicit back-port authorization (standing "decisions back-port to PRODUCT.md" rule). STILL PENDING owner/legal (NOT ratified): DPO appointment (contact remains placeholder bonitobonita24@gmail.com until a dedicated DPO mailbox exists), NPC registration / PIA, and per-feature lawful-basis fine-tuning. Controller = Powerbyte IT Solutions stands.
- [v329-compliance · 2026-06-20] q-v329-05 [A/technical] Prisma migration RUNTIME-DEFERRED — schema + regenerated client committed, but no migration SQL generated (no live DB to diff). Follow-up: owner runs `cd packages/db && pnpm exec prisma migrate dev --name v329_compliance` against dev DB, then promote. New compliance tables are unused by runtime until migrated (routers compile + typecheck against the generated client). WCAG 2.2 pass applied to compliance surfaces (privacy page, DSR settings, login: 44px targets, focus-visible rings, label associations, semantic tokens, skip-link). Remaining pre-existing a11y items logged for owner: AppShell.tsx:104 decorative img alt verify; OverlayNamePicker focus-trap unaudited.
- [v329-ratify · 2026-06-21] q-v329-06 [A/product-ratify] **OWNER-RATIFIED V32.9 compliance product values** (back-port of owner decision, authorized by conductor per the standing "decisions back-port to PRODUCT.md" rule; Rule-1 human-only edit waived for this back-port only). Ratified: (1) DSR statutory response window = **15 calendar days** (flips q-v329-02 from flagged-assumption to confirmed; matches `DSR_DUE_DAYS = 15` already in `dsr.ts` — no code change needed, the constant carried no "needs confirmation" caveat). (2) Retention schedule = financial/payroll **7y** · audit logs **5y** baseline · general operational data **3y**. Yelli reconciliation: Yelli has no payroll data; its AuditLog is retained **7y** (exceeds the 5y baseline) under the stricter PH BIR/SEC records-retention rule, CallSession 1y as operational data — existing code/PRODUCT.md values are compliant with the ratified schedule and were NOT downgraded. (3) WCAG **2.2 AA** reconciled in PRODUCT.md (NFR Accessibility + Design Identity key-constraint), aligning docs with code/footer already shipped at 2.2 AA. Files touched: `docs/PRODUCT.md` (NFR Data retention + Compliance + Accessibility lines; Design Identity key constraint), `docs/DECISIONS_LOG.md` (q-v329-02 + q-v329-04 + this entry). **STILL PENDING owner/legal (NOT ratified):** DPO appointment, NPC registration/PIA, per-feature lawful-basis fine-tuning. **Untouched future work:** `dsr.object`/`dsr.inform` TODO wires.
