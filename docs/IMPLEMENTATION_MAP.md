...
- 🚧 Phase 3.3 — IN PROGRESS (Wave 4/N complete, three Core User Flows walkable 2026-06-08)
  ...
  - Sim semantics gap (deferred to Phase 4 backend swap): `sim.devices.setRole` already audits with `{deviceId, role}` but lacks `from` field §11 mandates; Wave 4B layered a second fully-specified entry rather than refactor mid-wave. Collapse candidate for Wave 5 housekeeping bundle.
- ⏳ Wave 5 — Flow D Register Device (`OverlayNamePicker` + `device.first_join` audit): NEXT (likely paired with Flow E LAN-anonymous-admin Login per R7 if independent; optional housekeeping bundle: collapse double `device.role.assign` audit into single repo-internal emit with `from`-field)
- ⏳ Waves 6–10 — Remaining 5 §3 Core User Flows (F-I + Tenant Export): PENDING (one per wave, strict R2)
