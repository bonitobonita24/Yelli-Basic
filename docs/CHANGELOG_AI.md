...
- Agent: CLAUDE_CODE (Opus-inline, standing V32.1 env-structural swarm fallback — headless `claude -p`
  worker; sub-agent dispatch unreliable per lessons.md. Two manifest edits + one lockfile + sequential
  audit verification form one indivisible unit with no independent fan-out boundary — no parallel
  fan-out warranted regardless; R1 deviation accepted per the standing pattern.)
- Why: The Phase 5 `pnpm audit --audit-level=high` gate was blocked by 2 HIGH CVEs on `swarm/rebuild`:
  (1) GHSA-rcmh-qjqh-p98v — nodemailer ≤7.0.10 addressparser recursive-call DoS; (2) GHSA-gv7w-rqvm-qjhr
  — esbuild <0.28.1 NPM_CONFIG_REGISTRY RCE (missing binary integrity verification). Both must clear for
  the audit gate to pass.
- Files modified:
  - packages/jobs/package.json — `nodemailer` dependency `^6.9.16` → `^7.0.11` (the W5c email worker
    direct dep; the recursive-addressparser fix landed in 7.0.11). `@types/nodemailer` devDep
    `^6.4.17` → `^7.0.11` to match the v7 runtime types (required for typecheck). The v7 `createTransport`
    / `sendMail` / `SentMessageInfo` (`accepted`/`rejected`/`messageId`) API is unchanged from v6, so the
    email worker body (`src/workers/email.ts`) compiles and behaves identically — NO worker code edit
    needed (verified by a clean typecheck). Our worker passes structured `{from,to,subject,text,html}`
    options, never raw address strings, so it never exercised the vulnerable recursive parser path.
  - package.json — added two `pnpm.overrides`: `esbuild: ">=0.28.1"` (fixes the `apps/signaling`
    transitive toolchain dep; latest published is exactly the 0.28.1 patched floor) and
    `nodemailer: ">=7.0.11"` (forces the SECOND vulnerable nodemailer path — `apps/signaling >
    @auth/core > nodemailer@6.10.1`, an optional peer of @auth/core that signaling does not functionally
    use, it only consumes `@auth/core/jwt` — up to 7.x). The brief's scope item (1) named only the jobs
    path; the brief's stated GOAL ("confirm 0 high / 0 critical") required clearing this transitive too,
    via the same pnpm-override mechanism the brief prescribed for esbuild. Same CVE, same package, same
    fix version — minimal in-scope extension, not opportunistic refactor.
  - pnpm-lock.yaml — regenerated by `pnpm install`: nodemailer pinned to 7.0.13 on ALL paths (jobs direct
    + @auth/core transitive), esbuild deduped to 0.28.1; the nodemailer-6 dependency subtree removed.
- Schema/migrations: none.
- Audit policy: structured logs only, no AuditLog change (the email worker's audit posture is unchanged;
  this is a dependency-manifest change with no domain-event surface).
- Errors encountered/resolved: First audit pass after the jobs bump still showed 1 HIGH — the
  `@auth/core > nodemailer` transitive in apps/signaling was unaffected by the jobs dep bump. Resolved
  by adding the `nodemailer: ">=7.0.11"` pnpm override (same approach as esbuild). Final
  `pnpm audit --audit-level=high`: 0 high / 0 critical (5 remaining = 2 low + 3 moderate, all below the
  gate threshold).
- Validation all green: `pnpm install` ✓ (only the documented pre-existing tRPC/next-auth/aws-sdk peer
  warnings), prettier ✓ (manifests unchanged by format), turbo typecheck 7/7 (confirms the email worker
  compiles against nodemailer 7 + @types/nodemailer 7.0.11 with no code change), turbo lint 7/7,
  turbo test 2/2 (web + signaling suites), `next build` 2/2 (route table unchanged). HIGH/CRITICAL audit: 0/0.
...
- Agent: CLAUDE_CODE (Opus-inline, standing V32.1 env-structural swarm fallback — headless
  `claude -p` worker; sub-agent dispatch unreliable in this environment per lessons.md. The install
  banner + offline banner + SW tap-through bridge + the UUIDv7/IDB replay queue + provider all share
  the W6a Service-Worker/idempotency contracts + the Clay token surface + one ReplayQueue type
  surface = one cohesive PWA-chrome unit with no independent fan-out boundary — no parallel fan-out
  warranted regardless. R1 deviation accepted per the standing pattern.)
- Why: Land the W6b client deliverables the W6a split deferred (q-W6-01 [A]) — the PWA install
  banner (flow #19), the cached-shell offline/Reconnecting banner (flow #21), the SW→client
  incoming-call tap-through bridge (flow #20), and the client-side UUIDv7 offline mutation replay
  queue with reconnect flush (flow #21), all wired to the already-shipped W6a SW + `push` router +
  Valkey 24h-dedup primitive.
- SCOPE NOTE (resolved from authoritative artifacts — no escalation): the S5 scope sheet listed a
  `coturn` compose service and `release.yml` + Windows PowerShell scripts; both pre-resolved by the
  Brain queue (q-run9-S5-02 [A] / q-run9-S5-03 [A]) as out-of-scope template carryover — S5 ships
  ONLY the W6b client PWA surface. The W6a Service Worker already implemented the cached-shell offline
  strategy + a `notificationclick` handler; S5 corrects that handler to the LOCKED §20 contract
  (postMessage `{ type:'incoming-call', callSessionId }` to a focused client, not `client.navigate`)
  and adds the client bridge — it does NOT recreate the SW.
- Files added:
  - `apps/yelli/src/lib/pwa/uuidv7.ts` — RFC-9562 v7 generator (time-ordered, dependency-free) for
    the offline idempotency key.
  - `apps/yelli/src/lib/pwa/replay-queue.ts` — IDB-backed FIFO replay queue primitive (`ReplayQueue`
    + `idbStore`/`memoryStore`); monotonic enqueue clock guarantees same-ms FIFO order; stop-on-throw
    replay preserves order across reconnects; fail-safe to memory without IndexedDB.
  - `apps/yelli/src/lib/pwa/uuidv7.test.ts` + `…/replay-queue.test.ts` — 9 unit tests (format/version/
    variant/time-ordering/uniqueness; FIFO order, drain, stop-on-failure, retry-same-head).
  - `apps/yelli/src/components/pwa/replay-queue-provider.tsx` — React context owning the queue +
    reconnect-flush lifecycle + per-`type` executor registry; `pendingCount` drives the offline banner.
  - `apps/yelli/src/components/pwa/offline-banner.tsx` — `navigator.onLine`-driven cached-shell
    offline / "Reconnecting…" banner with queued-action count (Clay tokens, a11y `role=status`).
  - `apps/yelli/src/components/pwa/install-banner.tsx` — `beforeinstallprompt` intercept, 2nd-visit
    gate (`localStorage.yelli_visited`), 30-day snooze (`localStorage.yelli_install_snoozed_until`),
    iOS Safari A2HS-walkthrough fallback (inline — the spec's Settings page is the deferred
    ScreenTenantSettings), standalone suppression, Cloud-only `push.recordInstall` audit on accept.
  - `apps/yelli/src/components/pwa/service-worker-bridge.tsx` — client listener for the SW
    `{ type:'incoming-call', callSessionId }` message → soft `router.push('/app?incoming=…')`.
  - `apps/yelli/src/components/pwa/pwa-global-chrome.tsx` — root-mounted OfflineBanner + bridge.
- Files modified:
  - `apps/yelli/public/sw.js` — `notificationclick` now POSTs the focused client the §20 message
    (was `client.navigate`, a deviation); header strategy comment updated.
  - `apps/yelli/src/app/layout.tsx` — wrap the tree in `ReplayQueueProvider` + mount `PwaGlobalChrome`
    inside `Providers`.
  - `apps/yelli/src/app/(app)/layout.tsx` — mount `InstallBanner` above the shell (idle-directory scope).
- Schema/migrations: none. New deps: none (UUIDv7 + IDB hand-rolled — consistent with W6a's
  "uuid generation is client-side (W6b)" anticipation; zero external npm packages).
- Decisions/boundaries (documented, non-blocking): toast feedback ("Will retry when reconnected") is
  surfaced as inline banner text — no `<Toaster>` is mounted in the app, so the queued-count messaging
  lives in the offline banner instead (same intent, no sonner dependency). The WebSocket-specific
  "Reconnecting…/exponential-backoff/Retry-now/CALL-disable/directory-resync" half of flow #21 stays
  with the single `useSignaling` instance the device-home call-engine owns — the network-level offline
  banner + replay queue ship here; the socket-coupled pieces wire in the call-engine session. The
  replay queue's executor registry has no live producer yet (the enqueuing mutations live in the
  deferred settings/call screens) — it ships as a complete, unit-tested primitive (the `useSignaling`
  S1 precedent: full transport before its consuming screens).
- Errors encountered/resolved: the FIFO test caught a real ordering bug — `Date.now()` repeats within
  a millisecond and a UUIDv7's intra-ms order is random, so same-ms enqueues scrambled; fixed with a
  monotonic enqueue clock (`monotonicNow`) so the FIFO sort is deterministic and reload-safe.
- Validation: prettier ✓; web typecheck 0; web lint 0/0; web test 11/11 (was 2/2 — +9 PWA tests);
  `next build` ✓ (route table unchanged — components are mounted not routed; only the pre-existing
  non-fatal @prisma/client `export *` Turbopack warning, S2); turbo typecheck+lint 14/14.
...
- Commit: `feat(phase-4-W7): Design system finalization (Phase 3.3 deferrals #2-#4)`.
- Why: Execute the three Phase 3.3 design-system deferrals locked at Phase 3.3 close (DECISIONS_LOG: font loading, overlay heading semantics, token consumption) plus a `/design-review` regression pass (V32.5 INHERIT-not-REPLACE). On inspection, only #3 (font) has an actionable production target this session — see the re-deferral notes below.
- DEFERRAL #3 (font loading) — DONE. Inter is now self-hosted via `next/font/google`, replacing the runtime Google Fonts `@import`:
  - `apps/yelli/src/app/layout.tsx` — `import { Inter } from 'next/font/google'`; `const inter = Inter({ subsets: ['latin'], display: 'swap', variable: '--font-inter' })` (Inter is a variable font ⇒ no `weight` needed; the full 400–700 range the prototype loaded is available); `inter.variable` added to `<html className>`. Verified against Next 16 `next/font` docs (context7).
  - `apps/yelli/src/app/globals.css` — removed `@import url('https://fonts.googleapis.com/css2?family=Inter…')`; `font-family` now leads with `var(--font-inter)` then `'Inter'`/system fallbacks (existing `letter-spacing: -0.01em` preserved — Output Equivalence with the GREEN baseline).
  - `apps/yelli/next.config.ts` — CSP tightened (security win): dropped `https://fonts.googleapis.com` from `style-src` and `https://fonts.gstatic.com` from `font-src` — `next/font` self-hosts the face + `@font-face` stylesheet from `'self'`.
- DEFERRAL #2 (overlay eyebrow `<div>` → `<h2>`) — RE-DEFERRED (no production target). `OverlayIncomingCall`/`OverlayCallRoleAssign` exist ONLY in `prototype/`; the production UI port (W1b for device/admin overlays; the calling-UI port per q-W2b-03 for the incoming-call overlay) has not run, so there is no production heading to promote. Framework-derivable per the Phase 4 Parts 5-6 MODEL HOOK (production UI is built by porting the prototype; the heading-semantics fix applies DURING that port). Re-deferred to the overlay UI-port session. NOT applied to the signed-off prototype (Phase 3.3 CLOSED; INHERIT-not-REPLACE — the prototype is the human-approved baseline, not editable here; it also carries no `eyebrow` markup to change).
- DEFERRAL #4 (hex literal → CSS var / token consumption) — RE-DEFERRED (no clean target without unlocking `tokens.css`). The proper token consumption (shadcn primitives → CSS vars) is already in place from S4a-1/S4a-2. The residual production hex are NOT cleanly tokenizable: `viewport.themeColor: '#1a3a3a'` is build-time metadata (cannot reference a CSS var; mirrors `--color-brand-teal` but must stay literal); `--primary-foreground`/`--destructive-foreground: #ffffff` are on-dark whites with no dedicated token (`--color-surface` is a background — aliasing would be semantically wrong + fragile); `--border`/`--input: #e5e5e5` have NO matching token in the LOCKED `tokens.css` — tokenizing them requires ADDING a neutral/border token to the LOCKED design-token source, forbidden without authorization (Rule 1 / "do ONLY scope" / NEVER edit a LOCKED token source unilaterally). Re-deferred to a DECISIONS_LOG-authorized token-vocabulary session (or finalized during the overlay port alongside #2). LOCKED `tokens.css` + `tokens.ts` left untouched (parity test 2/2).
- `/design-review` regression (V32.5 INHERIT-not-REPLACE) — GREEN. Baseline body face in `docs/MOCKUP.jsx` + `docs/DESIGN.md` is Inter → preserved (next/font + `'Inter'` fallback + intact letter-spacing). No design-token drift (LOCKED source untouched; parity 2/2). `docs/DESIGN.md`/`docs/MOCKUP.jsx` not regenerated. (Note: the `design-auditor` skill is a no-rules stub — review performed manually against the baseline.)
- Files modified: `apps/yelli/src/app/layout.tsx`, `apps/yelli/src/app/globals.css`, `apps/yelli/next.config.ts` (+17/-7), `docs/STATE.md`, `docs/CHANGELOG_AI.md`.
- Validation all green: prisma generate ✓, web typecheck ✓ (0), web lint ✓ (0), prettier ✓ (3 touched files), web test ✓ (token-parity 2/2), `next build` ✓ (`ƒ Proxy (Middleware)`; only the pre-existing non-fatal @prisma/client `export *` Turbopack warning — S2, unrelated).
