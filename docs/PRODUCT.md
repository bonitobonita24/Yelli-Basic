...
Data retention: AuditLog 7 years (PH BIR / compliance norm). CallSession metadata 1 year. User accounts indefinite while active; soft-delete grace 7 days then hard delete. Device rows retained indefinitely while active; auto-archived after 90 days offline (hidden from default admin views, still queryable and restorable); hard-delete only on explicit admin removal. VerificationTokens auto-expire per type (1h reset / 7d invite / 24h verify). Retention schedule (owner-ratified 2026-06-21): financial/payroll records 7y · audit logs 5y baseline (Yelli's AuditLog is held 7y, exceeding the baseline under the stricter PH BIR/SEC records-retention rule) · general operational data 3y.
Compliance:     PH DPA (RA 10173) primary; data-subject requests (access / portability / rectification / erasure) honored within a 15-calendar-day statutory response window (owner-ratified 2026-06-21; NPC reasonable-period guidance). GDPR opt-in (export + delete on request) for any EU customer. No PCI-DSS scope in MVP. No HIPAA scope. SOC 2 / ISO 27001 deferred until enterprise customers ask. DPO appointment, NPC registration/PIA, and per-feature lawful-basis fine-tuning remain pending owner/legal action.
Accessibility:  WCAG 2.2 AA — framework R13 default + DICT MC 004; enforced by `accessibility-agents` / `a11y` skill pre-delivery checklist. Set `accessibility: wcag_aa` in inputs.yml.
...
Key constraint:     WCAG 2.2 AA accessibility
